...
export default function LeaderboardMoversPanel({
    ...
    const hasMovers = data.climbers.length > 0 || data.fallers.length > 0;

    return (
        <div className="w-full rounded border border-white/10 bg-black/60 p-3 backdrop-blur-sm">
            <div className="mb-2 text-xs text-white/60">Leaderboard Movers ({data.period})</div>
            {!hasMovers ? (
                        ...
                        <div>
                            ...
                            <div className="space-y-1">
                                {data.climbers.map((m) => (
                                    ...
                                    >
                                        <span className="text-white/70">{m.player_name}</span>
                                        <div className="flex items-center gap-2">
                                            <span className="text-white/30">{m.current_elo}</span>
                                            <span className="text-green-400">+{m.elo_change}</span>
                                        </div>
                                    </button>
                                ))}
                            </div>
                            ...
                            <div className="space-y-1">
                                {data.fallers.map((m) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-white/30">{m.current_elo}</span>
                                            <span className="text-red-400">{m.elo_change}</span>
                                        </div>
                                    </button>
                                ))}
                            </div>
